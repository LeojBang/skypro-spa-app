...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        course = Course.objects.get_or_create(
            name="Test Course", defaults={"description": "Test Course Description"}
        )[0]

        lesson = Lesson.objects.get_or_create(
            course=course,
            name="Test Lesson",
            defaults={"description": "Test Lesson Description"},
        )[0]
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully created {len(payments)} payments")
        )
